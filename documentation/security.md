...
I've added a password to my private SSH key (stored in 1Password), which is a little more annoying to use but means that if someone where to gain access to my key they would need the password to do anything meaningful with it. For example, to push to the repo the password is required. 
